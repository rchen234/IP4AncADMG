...
def contract_heur(n, icnodes, icparents, icweight, cutwt):
    ...
    # now look for cluster ineqs. First modify bidirected sets
    if len(bdir) > 0:
        cl = len(cnodes)
        for i in range(cl):
            cset = cnodes[i]
...
n = 5
gcnodes = [[0], [1], [2], [2], [3,4]]
gcweight = [1.0, 1.0, 0.5, 0.5, 1.0]
gcparents = [[[1,2]], [[2, 3]], [[]], [[3]], [[0], [1]]]
...
random.seed(1)
Cset = contract_heur(n+1, gcnodes, gcparents, gcweight,1.0)
print(Cset)
